...
- [x] Follow-up (GO): wire sequential best-of-N (seq3-style) into `run_separation_layout` as a sequential orchestration mode (component scheduling inside the user budget; cancellation across members via the existing kill registry; GUI exposure + budget presentation decided in that spec) — separate spec/plan/PR. (shipped — see the productization section below.)
...
- [x] P1: Worktree preflight + spec/plan/BACKLOG committed on branch
- [x] P2: Engine progress module (core/layout/progress.py)
- [x] P3: Engine sequential orchestration + stop-best-so-far (run_separation_layout)
- [x] P4: API — response flags, truthful-key partial caching, GET /layout-progress
- [x] P5: Frontend — quality-aware Stop + response types
- [x] P6: Frontend — progress polling + statusbar + QualityPanel copy
- [x] P7: Real-sparrow sequential integration + full suites green
- [x] P8: Docs (CLAUDE.md + BACKLOG ticks/outcome — NO PERFORMANCE.md), PR, final review (PR + final review complete at merge)
- Outcome: SHIPPED — Ultra best-of-N now sequential (seq3 policy, PR #23); Stop returns best completed member (truthful-key cached); GET /layout-progress + GUI run-k-of-N line; QualityPanel "Runs (keep best of N)" + total-time hint. Engine suite 275 passed (was 259); frontend 64 passed. No PERFORMANCE.md entry (feature round — this section is the record).
