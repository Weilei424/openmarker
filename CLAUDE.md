...
- `core/layout/ga.py` — Genetic Algorithm meta-heuristic driver (island model). `run_ga(...)` is a pure function taking its BLF evaluator as a callable (testable with a stub); mutation reuses `sa.py`'s swap/reverse/rotation-flip operators, crossover is Order Crossover (ordering) + per-gene uniform (rotations). Orchestration in `heuristic.py::_run_ga_phase` runs K independent island populations on the shared `ProcessPoolExecutor`, each seeded from the best-of-4 `WarmStart` pool. `GAConfig` holds the hyperparameters (tunable per-call via `auto_layout_polygon(ga_config=...)`, engine-Python-only). No cross-island pruning → deterministic per seed. Opt-in via `auto_layout_polygon(ga_generations > 0)`; mutually exclusive with clustering and with `sa_iterations > 0`. Uniform-weight `MUTATION_MOVE_WEIGHTS` default beats both the bar and SA on the canonical workload. See PERFORMANCE.md § 4.7.
- `core/layout/cancellation.py` — module-level flag + `CancellationError`. `is_cancelled()` checked between piece placements.
- `core/layout/separation.py` — Separation nesting engine, the GUI **"Ultra"** tier. `run_separation_layout()` converts pieces → grain-aligned + 90° axis-mapped jagua-rs JSON → bundled `sparrow.exe` subprocess (overlap-and-separate strip nester; MIT, on MPL-2.0 jagua-rs) → inverse axis-map reconstruction → hard-fail validation (grain / overlap / width / coverage) → `_compute_metrics`. Returns `auto_layout_polygon`'s `(placements, marker_length, utilization)` tuple. Grain is HARD-enforced both ways: per-item `allowed_orientations` is derived from grain mode (single → `[0]` no-flip / bi → `[0,180]` / no-grainline → cardinals) and the validator re-asserts grain on every placement. `n_seeds > 1` runs members SEQUENTIALLY (one 3-thread sparrow at a time, each at the full budget — the PR #23-validated seq3 policy; seeds `seed..seed+N−1`) and keeps the shortest VALID marker; Stop mid-run returns the best COMPLETED member (`stopped_early`), with progress reported via `core/layout/progress.py` (module-level single-flight snapshot; final snapshot on every exit path); `kill_current_sparrow()` lets `/cancel-layout` terminate the child(ren). **`warm_start=True` (default) seeds sparrow from a Fast-tier NFP-BLF layout passed via sparrow's `-i` flag as an `ExtSPOutput` JSON (`{**_instance_json, "solution": …}`; `_build_warm_start` + `_placements_to_jagua` = the exact inverse of `_reconstruct`, with a pure-translation guard). Built ONCE and shared across all best-of-N attempts; a Fast-layout failure degrades gracefully to a cold start, and a Stop during the prelude raises `CancellationError`. The Fast layout is an add-on prelude — sparrow still gets the full `budget_s`.** Binary vendored at `engine/vendor/sparrow/` (resolver ladder: env `OPENMARKER_SPARROW_PATH` → vendored → PyInstaller `_MEIPASS` → dev `tools/`). Beats GA +5.20% on sample_2×10; warm start adds ~−0.7% on the canonical workload (first sub-commercial markers). See PERFORMANCE.md §6 [2026-06-08] + [2026-06-12 round 2].
...
- `app/App.tsx` — wires all panels; manages fabric width (defaults to 1500 mm and resets on every import), grain mode, copies (a committed number, reset to 1 on import), layout-quality tier, and selection state. The Copies / Cached results / Time budget number inputs are `NumberField`s. Topbar shows "OpenMarker — Working on {filename}" once a DXF imports.
...
- `hooks/useAutoLayout.ts` — `POST /auto-layout` wrapper; `abort()` cancels in-flight fetch AND posts `/cancel-layout`. Quality-aware: ultra keeps the request open on Stop (returns best completed member); other qualities abort the fetch client-side (→ aborted); a 499 is consumed only by the ultra path.
