...
- `api/main.py` — FastAPI app, CORS middleware. Routes:
  ...
  - `POST /auto-layout` → pieces + fabric + grain + `quality` tier in, placements + marker_length + utilization out. `quality`: `fast` = NFP-BLF warm-start; `better`/`best` = GA (180s/420s budgets); `ultra` = separation engine (sparrow) at a user budget (`ultra_budget_s`) + best-of-N seeds (`ultra_seeds`). Runs in a worker thread via `run_in_threadpool` so other endpoints stay responsive.
...
- `core/layout/separation.py` — Separation nesting engine, the GUI **"Ultra"** tier. `run_separation_layout()` converts pieces → grain-aligned + 90° axis-mapped jagua-rs JSON → bundled `sparrow.exe` subprocess (overlap-and-separate strip nester; MIT, on MPL-2.0 jagua-rs) → inverse axis-map reconstruction → hard-fail validation (grain / overlap / width / coverage) → `_compute_metrics`. Returns `auto_layout_polygon`'s `(placements, marker_length, utilization)` tuple. Grain is HARD-enforced both ways: per-item `allowed_orientations` is derived from grain mode (single → `[0]` no-flip / bi → `[0,180]` / no-grainline → cardinals) and the validator re-asserts grain on every placement. `n_seeds > 1` runs parallel attempts and keeps the shortest VALID marker; `kill_current_sparrow()` lets `/cancel-layout` terminate the child(ren). Binary vendored at `engine/vendor/sparrow/` (resolver ladder: env `OPENMARKER_SPARROW_PATH` → vendored → PyInstaller `_MEIPASS` → dev `tools/`). Beats GA +5.20% on sample_2×10. See PERFORMANCE.md §6 [2026-06-08].
...
- `components/sidebar/QualityPanel.tsx` — layout-quality selector (NFP-BLF / Genetic Algorithm quick & thorough / Separation). The Separation tier reveals a time-budget `NumberField` (360–1500 s) and a best-of-N seeds radio.
