...
- `hooks/useAutoLayout.ts` — `POST /auto-layout` wrapper; `abort()` cancels in-flight fetch AND posts `/cancel-layout`. Quality-aware: ultra keeps the request open on Stop (returns best completed member); other qualities abort and return 499.
